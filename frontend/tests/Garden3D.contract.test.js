...
describe('3D garden MVP contract', () => {
  ...
  test('GardenScene owns the Three.js scene and click-selection contract', () => {
    ...
    expect(source).toContain('function createGardenDioramaScene')
    expect(source).toContain('function createPaperTray')
    expect(source).toContain('function createGroundPatches')
    expect(source).toContain('function createStonePath')
    expect(source).toContain('function createCheerfulFlowerZone')
    expect(source).toContain('function createMemoryTreeZone')
    expect(source).toContain('function createCalmPondZone')
    expect(source).toContain('function createTransformingVinesZone')
    expect(source).toContain('function createWeatherLayer')
    expect(source).toContain('function createFloatingDetails')
  ...
  test('GardenScene uses a fixed orthographic stage camera with zoom-only controls', () => {
    ...
    expect(source).toContain('FRUSTUM_SIZE = 7.2')
    expect(source).toContain('HOME_CAMERA_POSITION = new THREE.Vector3(5.8, 5.0, 6.4)')
    ...
    expect(source).toContain('controls.minZoom = 0.85')
    expect(source).toContain('controls.maxZoom = 1.45')
  ...
  test('GardenScene includes paper-model terrain and selected plant effects', () => {
    ...
    expect(source).toContain('weatherLayer')
